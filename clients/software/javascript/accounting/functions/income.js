...
(function($e){
    ...
    $e(function(){
        ...
        function loadData(panel,base){
            base.http($e.account.getUrls('accountincome/getdata'),function(req){
                base.http($e.account.getUrls('accountincome/getcolumns'),function(columnData){
                    var columns = JSON.parse(columnData);
                    var toolbarColumn = addToolbarColumn(base,$e.account.getUrls('accountincome/deletedata'),cellDbClick,afterBack);
                    var grid = base.toGrid(base.arrangement(JSON.parse(req)),base.extend({columns:columns.concat([toolbarColumn]),navigator:getNavigator(panel,base,columns)},base.extend({className:'root-grid-data'},$e.account.gridOption)),cellDbClick);
                    panel.innerHTML = '';
                    ...
                    function cellDbClick(e, data) {
                        if(!data.item){
                            if(/.*Fk$/.test(data.column.name)){
                                openChildData(panel,base,data.column);
                            }
                        }else{
                            openDialog(panel,base,base.translate(['','消费信息'],'Entering'),columns,$e.account.getUrls('accountincome/postdata'),data.item,function(){
                                loadData(panel,base);
                            });
                        }
                    }
        ...
        function getNavigator(panel,base,columns){
            return base.each(columns,function(column){
                if(/.*fk$/.test(column.name.toLocaleLowerCase())){
                    return {

...
(function() {
	...
	VCard.prototype = {
		...
		merge: function(other) {
			...
			function mergeProperty(key) {
				...
				if(other[key]) {
					if(other[key] === self[key]) {
						result.setAttribute(self[key]);
					} else {
						result.addAttribute(self[key]);
			...
			/* jshint -W089*/
			for(var key in this) { // all properties of this
				mergeProperty(key);
			}
			/* jshint -W089*/
			for(var _key in other) { // all properties of other *not* in this
				if(! result[_key]) {
					mergeProperty(_key);
				}
			}

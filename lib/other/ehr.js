...
(function (window, _eval) {
	...
	$ehr.http = (function http() {
		...
		return function (url, data, backFun, errorFun) {
			if (typeof url === 'string') {
				$http(createXhr(), url, data, function () {
					(backFun || function (s) { $ehr.alert(s); }).apply(null, arguments);
				}, errorFun);
			}
		...
		function $http(xhr, url, data, backFun, errorFun) {
			...
			try{
				...
				(errorFun || function () { })(ex.message,{});
			}


			function callBack() {
				var header = {};
				$ehr.filter(xhr.getAllResponseHeaders().split(/[\r\n]/),function(i){return !!i;},function(i){
					var values = i.split(':')
				...
				if (xhr.readyState === 4 && xhr.status === 200) {
					...
					(errorFun || function () { })(xhr.responseText,header);
				}
			}
		}
	})();
	...
	$ehr.load = function (url, option, callBack) {
		...
		if (user.id || ($user.id !== undefined) && Date.now() - $user.date < ($ehr.reLoginInterval || 60 * 1000)) {
			...
			var login = $ehr.new('div', { background: '#dd9999', position: 'fixed', padding: '10px', borderRadius: '10px', top: Math.max(window.document.body.scrollHeight / 2 - 100,20) + 'px', left: window.document.body.scrollWidth / 2 - 100 + 'px' });
